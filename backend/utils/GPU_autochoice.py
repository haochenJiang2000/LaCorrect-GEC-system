...
def parse(line, qargs):
    ...
    power_manage_enable = lambda v: (not 'Not Support' in v)  # lambda表达式，显卡是否支持power management（笔记本可能不支持）
...
def query_gpu(device_list, qargs=[]):
    '''
    qargs:
        query arguments
    return:
    ...
    gpus = [parse(line, qargs) for line in results]
    for gpu in gpus[:]:
        # print(int(gpu['index']))
        # print(int(gpu['index']), device_list)
        if int(gpu['index']) not in device_list:
            # print(int(gpu['index']))
            gpus.remove(gpu)
    return gpus
...
class GPUManager():
    ...
    def __init__(self, GPU_LIST, qargs=[]):
        '''
        '''
        self.qargs = qargs
        self.gpus = query_gpu(GPU_LIST, qargs)
        self.device_list = GPU_LIST
        for idx, gpu in enumerate(self.gpus[:]):
            self.gpus[idx]['specified'] = False
    ...
    def auto_choice(self, mode=0):
        ...
        自动选择最空闲GPU,返回索引
        '''
        for old_infos, new_infos in zip(self.gpus, query_gpu(self.device_list, self.qargs)):
            old_infos.update(new_infos)
        # print(self.gpus)
        unspecified_gpus = [gpu for gpu in self.gpus if not gpu['specified']]

        if not unspecified_gpus:
            print("=========================")
            # print(self.gpus)
            for idx, gpu in enumerate(self.gpus[:]):
                self.gpus[idx]['specified'] = False
