...
from model.gector.seq2label.seq2labels_model import Seq2Labels
from model.utils.helpers import PAD, UNK, get_target_sent_by_edits, START_TOKEN, split_char
...
class GecBERTModel(object):
    def __init__(self,
        ...
        :param lowercase_tokens: 是否全部转小写
        :param log: 是否输出日志
        ...
        self.vocab = Vocabulary.from_files(vocab_path) if os.path.exists(vocab_path) else vocab  # 从文件里读取词典
        self.log = log
        ...
        if self.mode == "word":
            self._word_indexer = {"tokens": SingleIdTokenIndexer(namespace="tokens")}
        self._pinyin_indexer = {'pinyin_tokens': SingleIdTokenIndexer(namespace='pinyin_tokens')}
        ...
        for model_path, weights_name in zip(model_paths, weights_names):  # 遍历所有模型保存的参数文件，重新获取模型对象（反序列化）
            ...
            # weights_name = "/home/zhenghuali/zhangyue/GECToR_Chinese/bert-base-chinese"  # 调整模型名称，加上模型的一些信息，例如是否在转小写后训练
            self.indexers.append(self._get_indexer(weights_name, special_tokens_fix))  # 获取模型对应的indexer
            pinyin_embs = None
            if self.pinyin:
                pinyin_embs = BasicTextFieldEmbedder({"pinyin_tokens": Embedding(embedding_dim=200,
    ...
    # Todo 这个函数有问题，应该作者是在还没加上ensemble时写的，忘记修改以适配ensemble的情况了
    def _restore_model(self, input_path):
        """
        从本地存储的模型参数文件，恢复模型
        :param input_path: 本地模型文件路径
        """
        loaded_model = {}
        ...
        for model_path in filenames:
            ...
            own_state = self.model.state_dict()  # Todo 无self.model
    ...
    def _get_embbeder(self, weigths_name, special_tokens_fix):
        ...
        :param weigths_name: 模型名称
        :param special_tokens_fix: 是否需要对MLM模型的特殊标记token化，如[CLS]等
        :return: 模型对应的embbeder
        """
        bert_token_emb = PretrainedTransformerEmbedder(model_name=weigths_name, last_layer_only=True,
        ...
        # GECToR使用了allennlp的BasicTextFieldEmbedder，来对输入句子的token域进行嵌入表示。
        # 传统的BasicTextFieldEmbedder，其token_embedders参数只需要使用allennlp自带的Embedding类即可（例如做词嵌入、词性嵌入等，相当于一个全连接embedding层）。
        # 为了采用预训练类BERT模型作为encoder，作者编写了PretrainedBertEmbedder这个类，为BERT定制化嵌入操作。（用BERT对token嵌入需要进行一些额外处理，例如subword算法等）
        return text_field_embedder

    def _get_indexer(self, weights_name, special_tokens_fix):
        """
        获取模型对应的indexer（编号器，用于完成token<—>id的转换）
        :param weights_name: 模型名称
        :param special_tokens_fix: 是否需要对MLM模型的特殊标记token化，如[CLS]等
        :return: 模型对应的indexer
        """
        bert_token_indexer = PretrainedTransformerIndexer(model_name=weights_name, namespace="bert")
        ...
        with torch.cuda.device(self.cuda_device):
            ...
            for indexer in self.indexers:  # 遍历各模型对应的indexer（模型ensemble的需要，不同模型的indexer不相同，因为采用了不同的预训练模型）
                ...
                for sequence in token_batch:  # 遍历token_batch中的所有句子
                    ...
                    pinyin_tokens = None
                    if self.pinyin:
                        pinyin_tokens = [Token("<MASK>")]
                        for token in tokens:
                            py = lazy_pinyin(token, errors="ignore")
                            if not py:
                                pinyin_tokens.append(Token("<MASK>"))
                            else:
                                pinyin_tokens.append(Token(py[0]))
                    if self.mode == "word":  # 词模式
                        char_tokens = [Token('$START')] + [Token(split_char(token)[0]) for token in tokens]
                        tokens = [Token(token) for token in ['$START'] + tokens]  # 进行allennlp的Token化处理
                        if self.pinyin:
                            batch.append(Instance({'tokens': TextField(tokens, self._word_indexer),
                                                   'char_tokens': TextField(char_tokens, indexer),
                                                   'pinyin_tokens': TextField(pinyin_tokens, self._pinyin_indexer)}))
                        else:
                            batch.append(Instance({'tokens': TextField(tokens, self._word_indexer),
                                                   'char_tokens': TextField(char_tokens, indexer)}))
                    elif self.mode == "char":  # 字模式
                        tokens = [Token(token) for token in ['$START'] + tokens]  # 进行allennlp的Token化处理
                        if self.pinyin:
                            batch.append(Instance(
                                {'tokens': TextField(tokens, indexer),
                                 'pinyin_tokens': TextField(pinyin_tokens, self._pinyin_indexer)}))
                        else:
                            batch.append(Instance(
                                {'tokens': TextField(tokens, indexer)}))
                    # 进行allennlp的instance化处理，获得一个句子的实例。这个实例包含一个tokens域，对原始token（word）采用BERTIndexer编号。主要是为了使用BERT等模型的subword算法。
                    # 以句子：['I', 'likes', 'to', 'swimming.']为例，
                    # indexer为word-token编号结果：{'bert': [101, 109, 9272, 12426, 1942, 146, 7407, 1106, 5947, 28138, 102], 'bert-offsets': [1, 5, 6, 7, 8], 'mask': [1, 1, 1, 1, 1]}
                    # 其中，indexer['bert']是这个句子的subword-token编号序列，indexer['bert-offsets']是我们需要预测输出的位置序列（论文中写的是以每个word首个或最后一个subword作为整个word的表示，进行预测）
                    # indexer['mask']是防止计算为了让长度一致进行的padding
                    # 如果不需要subword算法，这里其实只要用最基本的singleIdIndexer就可以了（一个token一个编号）
    ...
    def _convert(self, data):
        ...
        d_tags_idx = torch.max(d_tags_class_probs,
                             dim=-1)[1]
        # all_class_probs[d_tags_idx == 1][:, 0] = 0
        # test = all_class_probs[d_tags_idx == 1][:,0]
        ...
        else:  # 直接使用近似维特比算法进行序列解码
            idx = data[0]['predict_labels'].tolist()
