...
def split(data, min_len, flag = 0):
    ...
    '''
    split_data = []
    for page, seq in enumerate(data):
        # 统一符号格式，去除特殊符号
        seq = seq.replace("?", "？")
        seq = seq.replace("!", "！")
        seq = seq.replace(",", "，")
        ...
        if flag:
            ...
            for i in range(len(seq_list2)):
                seq_list2[i] = [page + 1, seq_list2[i]]
        else:
            for i in range(len(seq_list2)):
                seq_list2[i] = [0, seq_list2[i]]
        for i in seq_list2:
            # todo:希望去掉 (1) (2) 这种序号，但直接使用lstrip会导致去掉了句首数字，如 1998年...
            # i = i.lstrip("()（）123456789 ")
            if re.match(r'[(（]\d+[)）]', i[1]):
                i[1] = i[1][re.match(r'[(（]\d+[)）]', i[1]).span()[1]:]
...
def data_reader(file_path):
    ...
    # cur = conn.cursor()
    # file_path = cur.execute("SELECT file_path FROM commit_record WHERE file_id = %s" % file_id).fetchone()[0]
    if file_path[-3:] == 'pdf':
...
def preprocess(file_id, json_path, doc=0, min_len=10):
    '''
    ...
    # 分句
    split_data = split(data, 10, flag)

    # 预处理
    processed_data = []
    for page, line in split_data:
        ...
        if re.search('[a-zA-Z0-9:：，./-]{24}', line):
            continue
