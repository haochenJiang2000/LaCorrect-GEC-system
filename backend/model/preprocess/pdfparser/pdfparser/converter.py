...
if list(map(int, fitz.VersionBind.split("."))) < [1, 19, 0]:
    raise SystemExit("PyMuPDF>=1.19.0 is required for pdfparser.")
...
class Converter:
    ...
    * Finally, generate docx with ``python-docx``.
    ...
    @property
    def fitz_doc_length(self): return len(self._fitz_doc)
    ...
    def default_settings(self):
        ...
        return {
            ...
            # 去重阈值置为1.1，即不进行去重
            'line_overlap_threshold'         : 1.1,    # [0,1] delete line if the intersection to other lines exceeds this value
    ...
    def convert(self, start:int=0, end:int=None, pages:list=None, **kwargs):
        ...
        Refer to :py:meth:`~pdfparser.converter.Converter.default_settings` for detail of 
                ...
                for section in sections:
                    for block in section.blocks:
                        ...
                        if isinstance(block, TextBlock):
                            yield (i-1, block.bbox, block.text)
                        elif isinstance(block, TableBlock):
                            for rows in block._rows:
                                for row in rows:
                                    if not row:
                                        continue
                                    yield (i-1, row.bbox, row.text)
