...
import torch
import model.postprocess.ChERRANT.modules.tokenization as tokenization
    ...
    def split_char(self, input_strings: List[str]) -> List:
        ...
        for input_string in input_strings:
            if not self.segmented:  # 如果没有被分字，就按照每个字符隔开（不考虑英文标点的特殊处理，也不考虑BPE），否则遵循原分字结果
                tokenizer = tokenization.FullTokenizer(vocab_file="./model/postprocess/ChERRANT/modules/vocab.txt", do_lower_case=False)
                input_string = input_string.strip()
                input_string = input_string.replace(" ", "")
                input_string = tokenization.convert_to_unicode(input_string)
                tokens = tokenizer.tokenize(input_string)
                segment_string = ' '.join(tokens)
            else:
