...
def main():
    ...
    hyp_m2 = open(args.hyp, "r", encoding='utf-8').read().strip().split("\n\n")[args.start:args.end] if args.start is not None and args.end is not None else open(args.hyp, "r", encoding='utf-8').read().strip().split("\n\n")
    ref_m2 = open(args.ref, "r", encoding='utf-8').read().strip().split("\n\n")[args.start:args.end] if args.start is not None and args.end is not None else open(args.ref, "r", encoding='utf-8').read().strip().split("\n\n")
